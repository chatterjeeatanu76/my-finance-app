...
import {
  ...
  Home, BarChart3, User, X, Trash2, Loader2, AlertCircle, CheckCircle
} from 'lucide-react'
import {
...
export default function FinanceApp() {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      showToast('error', 'Failed to load transactions: ' + error.message)
    } finally {
      setLoading(false)
    }
  }
  ...
  const totalIncome = useMemo(() =>
    transactions.filter(i => i.type === 'income').reduce((s, i) => s + Number(i.amount), 0),
    [transactions])
  const totalExpense = useMemo(() =>
    transactions.filter(i => i.type === 'expense').reduce((s, i) => s + Number(i.amount), 0),
    [transactions])
            ...
            <div className="hidden md:flex items-center justify-center lg:justify-end gap-3 flex-wrap">
              {['home', 'reports', 'profile'].map(page => (
                <button key={page} onClick={() => setActivePage(page)}
                  className={`px-6 py-3 rounded-2xl transition-all duration-300 font-semibold capitalize ${
                    activePage === page
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 bg-gradient-to-b from-zinc-900 to-zinc-950 backdrop-blur-2xl rounded-[32px] p-6 border border-white/10 shadow-[0_20px_80px_rgba(0,0,0,0.45)] h-fit sticky top-6">
              <div className="flex items-center gap-2 mb-6">
                <Plus /><h2 className="text-2xl font-semibold">Add Transaction</h2>
              </div>
              <div className="flex bg-zinc-800 rounded-2xl p-1 mb-4">
                {['expense', 'income'].map(t => (
                  <button key={t} onClick={() => setType(t)}
                    className={`flex-1 py-3 rounded-2xl font-medium transition-all capitalize ${
                      type === t
              ...
              <div className="space-y-4">
                <input
                  ...
                  type="date"
                  className="w-full bg-zinc-800/70 border border-zinc-700 rounded-2xl px-4 py-4 outline-none text-white"
                />
                <input
                  value={title}
                  onChange={e => setTitle(e.target.value)}
                  placeholder="Transaction title"
                  className="w-full bg-zinc-800/70 border border-zinc-700 rounded-2xl px-4 py-4 outline-none text-white placeholder:text-zinc-500"
                />
                <input
                  value={amount}
            ...
            <div className="lg:col-span-2 bg-white/5 backdrop-blur-xl rounded-[32px] p-6 border border-white/10 shadow-2xl">
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  ...
                  className="bg-white/10 border border-white/10 px-4 py-2 rounded-xl text-sm hover:bg-white/20 transition-all flex items-center gap-2"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white/5 backdrop-blur-xl rounded-[32px] p-6 border border-white/10 shadow-2xl">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button onClick={() => setReportView(reportView === 'table' ? 'charts' : 'table')}
                    className="bg-white/10 border border-white/10 p-3 rounded-2xl hover:bg-white/20 transition-all">
                    <PieChart size={20} />
                  </button>
                </div>
          ...
          <div className="max-w-2xl mx-auto bg-white/5 backdrop-blur-xl rounded-[32px] p-6 border border-white/10 shadow-2xl">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-zinc-400 mt-2">{profileData.role}</p>
              </div>
              <button
                onClick={() => setIsEditingProfile(!isEditingProfile)}
                className="bg-white text-black px-5 py-3 rounded-2xl font-semibold hover:bg-zinc-200 transition-colors"
              >
                ...
                <div key={key} className="bg-zinc-800 rounded-2xl p-5 border border-zinc-700">
                  ...
                  {isEditingProfile ? (
                    <input
                      ...
                      className="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-3 outline-none text-white"
                    />
                  ) : (
        ...
        <div className="fixed bottom-4 left-4 right-4 md:hidden bg-black/70 backdrop-blur-2xl border border-white/10 rounded-3xl px-6 py-4 shadow-[0_20px_80px_rgba(0,0,0,0.55)]">
          <div className="flex items-center justify-between">
            {[
              ...
              <button key={page} onClick={() => setActivePage(page)}
                className={`flex flex-col items-center gap-1 text-xs ${activePage === page ? 'text-white' : 'text-zinc-500'}`}>
                {icon}{label}
              </button>
            ))}
          </div>
